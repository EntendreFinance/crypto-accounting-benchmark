...
The most consequential decision in this design is to measure **reproduction of a
recorded treatment**, not correctness in the abstract.
...
The target is therefore *the entry this organization actually recorded, chosen from
this organization's own chart of accounts*. That ground truth exists in a ledger
independently of the benchmark. Nobody had to invent it, and no reviewer has to agree with it
...
person, counterparty, venue name, bank, account name, account number, address,
transaction identifier, asset ticker, chain, amount, quantity and timestamp is replaced
by a synthetic value, consistently across the whole dataset. Generic accounting
terminology and some public categorical labels are published as recorded, among them
the source system's exchange enum, which names a real venue in 23 tasks. These measures
do not establish complete anonymity.
...
`inferable_from_visible_evidence` for exactly this reason: a task that is not
inferable does not survive admission.

**No near-duplicate decisions.** Tasks are grouped by the accounting decision they
actually test - the deciding non-wallet account plus the entry shape - and the number
admitted per group is capped. Without a cap, the easiest and most numerous pattern in a
...
what bounds the dataset at 118 tasks. Growing it means finding new *kinds* of decisions,
not more rows.
...
rather than an accident: coverage is uneven. Rubric families range from 37 tasks down
to 5. That is what a rule-based admission process produces from real data. The alternative - synthesizing filler tasks to balance
...
Before a task is admitted it has to survive automated checks:
...
- no identity-bearing value survived the transformation, apart from the public
  categorical labels the dataset documents as retained.

The last two are where defects concentrate. Decimal handling in particular is
...
**Set the balance cap at the first task.** A per-decision cap shapes the distribution
only for the tasks admitted under it, so a cap applied to an already large dataset
inherits whatever imbalance is in it. Setting it at the start yields a smaller but more
evenly distributed dataset for the same effort.

**Grade by account identity, not account name.** Answers here are matched on
...
only, removes a class of false negatives that has nothing to do with accounting skill.
...
turn on a single decisive signal and others require combining several, and that
distinction is the most useful thing a difficulty label can carry. Capture it at
admission time, where the evidence is still in front of you.
